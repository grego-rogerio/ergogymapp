...
class FuncaoEdit extends Component {
  ...
  async componentDidMount() {
    if (this.props.match.params.id !== 'new') {
      try {
        const response = api.get(`/funcao/${this.props.match.params.id}`).then( res => {this.setState({ item: res.data})
        console.log("Response Dimount Funcao ",response);  
      });

      const resp_setor = api.get(`/setor`).then( res => {this.setState({ setores: res.data})
      console.log("Response Dimount Funcao ",resp_setor);  
    });
      ...
      <Container>
        ...
        <Form onSubmit={this.handleSubmit}>
          ...
          <FormGroup>
              ...
              <select value={item.setor} onChange={item.setor}>
            {
                this.state.setores.map(function (itemSetor) {
                    return <option key={itemSetor} value={itemSetor}>{itemSetor.nome}</option>;
                })
            }
        </select>
          </FormGroup>
